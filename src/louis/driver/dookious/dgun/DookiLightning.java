...
//		_enemySurfBufferSet = new StatBufferSet();
//		_enemySurfBufferSet.addStatBuffer(
//			new EnemySurfBuffer(BINS), weight = 1);
//		_bulletHitRegisters.add(_enemySurfBufferSet);
...
/*	
	public void refreshEnemySurfPerspective() {
		int tightMatchDistanceThreshold = 50;
		Wave surfWave = DUtils.findClosestWave(_waves, ScanLog.enemyLocation(), 
			_robot.getTime(), DUtils.SURFABLE_WAVE, DUtils.FIRING_WAVE,
			tightMatchDistanceThreshold);
		
		if (surfWave == null) { return; }
		
		BotScan scan = surfWave.targetScan;
		BotScan enemyScan = scan.getInverseScan();

		double timeToImpact = DUtils.bulletTicks( 
				surfWave.sourceLocation.distance(ScanLog.enemyLocation()) -
				surfWave.distanceTraveled(_robot.getTime()),
			surfWave.bulletPower);
		
//		int mostDangerousEnemySurfBin = _enemySurfBufferSet.getBestBin(scan, 
//			enemyScan, _bulletPower, BINS);
		int safestEnemySurfBin = _enemySurfBufferSet.getWorstBin(scan, 
				enemyScan, _bulletPower, BINS);
		int currentEnemyBin = 
			surfWave.guessFactorIndex(ScanLog.enemyLocation());	
		
		if (timeToImpact > 3 &&
//			mostDangerousEnemySurfBin < currentEnemyBin &&
			safestEnemySurfBin > currentEnemyBin) {
			if (_nextWave.orientation == surfWave.orientation) {
				_antiSurferGun.setToPositiveGuessFactors();
			} else {
				_antiSurferGun.setToNegativeGuessFactors();
			}
		} else if (timeToImpact > 3 &&
//			mostDangerousEnemySurfBin > currentEnemyBin &&
			safestEnemySurfBin < currentEnemyBin) {
			if (_nextWave.orientation == surfWave.orientation) {
				_antiSurferGun.setToNegativeGuessFactors();
			...
			_antiSurferGun.setToAllGuessFactors();
		}
	}
*/
