...
public class Wave {
	...
	public GuessFactorWindowSet guessFactorWindow(Point2D.Double targetLocation) {
		...
		return new GuessFactorWindowSet(guessAngle / maxEscapeAngle,
			guessAngleLow / maxEscapeAngle, guessAngleHigh / maxEscapeAngle);
	}
	...
	public double uncachedPreciseEscapeAngle(boolean positiveGuessFactor) {
		...
		double withSmoothingEscapeAngle = Double.NEGATIVE_INFINITY;
		
		if (hitWall) {
			double wallSmoothingStick = 130;
			int purelyPerpendicularAttackAngle = 0;
