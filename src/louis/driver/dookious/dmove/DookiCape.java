...
public class DookiCape {
    ...
    private static final double DEFAULT_WALL_STICK = 160;
    private static final double RAMMER_WALL_STICK = 130;
    private static final double FEARFUL_DISTANCING_EXPONENT = 4;
    private static final double NORMAL_DISTANCING_EXPONENT = 1.5;
    private static final double BOT_HALF_WIDTH = 18;
    private static final double SURF_ORIENTATION_VELOCITY_THRESHOLD = 0.1;
    private static final double TYPICAL_DISTANCE = 465;
...
//		drawPoint(predictedState.location, drawColor);
		...
		double nextCounterClockwiseDanger = checkDanger(predictedState,
			...
			surfableWaveIndex + 1, recursionLevels);
		...
		if (_robot.getRoundNum() >= 20) {
            setFlattener(
            		normalizedEnemyHitPercentage() > ( 
            			(powerTwoHitPercentageThreshold = 9.8)) &&
            		_weightedEnemyShotsFired > 100);
		} else if (_robot.getRoundNum() >= 10) {
            setFlattener(
            		normalizedEnemyHitPercentage() > ( 
            			(powerTwoHitPercentageThreshold = 10.5)) &&
            ...
            setFlattener(
            		...
            		normalizedEnemyHitPercentageLastRound() >
            			(powerTwoHitPercentageThreshold = 14) &&
